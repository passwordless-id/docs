...
an attacker could not impersonate you since only the public key is stored there.
...
As hackers know, "an authentication service is as secure as its recovery mechanisms".
For example, it would be pointless if the recovery mechanism was based on a password or questions/responses.
...
You can have multiple authorized devices and password manager authenticators. 
...
For example, when you use "Sign in with Google", all websites implementing this login will have access to your global user ID.
This enables them to cross-reference your ID for tracking purposes and a profile activity.
Such a thing is impossible with Passwordless.ID, since all websites get distinct IDs for the same user.

### No third-parties involved

The data is safely kept here. It is not shared, sold or otherwise transmitted to any third-parties.
...
An anonymous "reputation" associated to users helps to determine their thrustworthiness.
...
while it decreases upon complains. The whole in a privacy-preserving way.
